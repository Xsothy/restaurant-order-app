...
import type { CategoryId } from "@template/domain/Category"
import { Category, CategoryAlreadyExists, CategoryNotFound } from "@template/domain/Category"
...
export class CategoriesRepo extends Effect.Service<CategoriesRepo>()(
    "Categories/Repo",
    {
        effect: Effect.gen(function*() {
            ...
            const deleteById = (id: CategoryId) =>
                findById(id).pipe(
                    Effect.andThen((category) => repo.delete(category.id))
                )

            const updateCategory = (id: CategoryId, name: string) =>
                findById(id).pipe(
