...
import { accessTokenFromRedacted } from "@template/domain/AccessToken"
import { policyUse, Unauthorized, withSystemActor } from "@template/domain/Policy"
import { CurrentUser, UserId, UserNotFound } from "@template/domain/User"
        ...
        Effect.gen(function*() {
            ...
            return handlers
                ...
                .handle("loginUser", ({ payload }) =>
                    accounts.loginUser(payload).pipe(
                        withSystemActor,
                        Effect.tap((user) =>
                            HttpApiBuilder.securitySetCookie(
                                Authentication.security.cookie,
